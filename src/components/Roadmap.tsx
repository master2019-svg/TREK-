...
export default function Roadmap() {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] gap-4">
        ...
        <p className="text-zinc-500 font-medium">Mapping out your adventure...</p>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] text-center max-w-md mx-auto">
        <div className="w-20 h-20 bg-zinc-100 rounded-3xl flex items-center justify-center mb-6">
          <MapIcon className="text-zinc-400 w-10 h-10" />
        </div>
        <h2 className="text-2xl font-display font-bold mb-3">Your Roadmap Awaits</h2>
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 h-[calc(100vh-12rem)]">
      <div className="xl:col-span-2 rounded-[2.5rem] overflow-hidden shadow-2xl border-8 border-white relative">
      ...
      <div className="overflow-y-auto pr-4 space-y-6 custom-scrollbar">
        <div className="flex items-center justify-between mb-8">
          <h3 className="text-2xl font-display font-bold">Itinerary</h3>
        ...
        <div className="space-y-4">
          {roadmap.data.map((item, idx) => (
            <motion.div
              ...
              className="glass p-6 rounded-3xl border border-zinc-100 relative group hover:border-zinc-300 transition-colors"
            >
              <div className="absolute -left-3 top-1/2 -translate-y-1/2 w-6 h-6 bg-zinc-900 text-white text-[10px] font-bold flex items-center justify-center rounded-full shadow-lg">
                {idx + 1}
              </div>
              
              <div className="flex items-start justify-between mb-4">
                <div>
                  <h4 className="font-bold text-zinc-900 group-hover:text-zinc-700 transition-colors">{item.place.name}</h4>
              ...
              {item.next_destination && (
                <div className="mt-4 pt-4 border-t border-zinc-50 flex items-center gap-3 text-zinc-400">
                  <div className="w-1.5 h-1.5 bg-zinc-200 rounded-full" />
                  <div className="flex-1 h-px bg-zinc-100" />
                  <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-zinc-400">
                    Next stop
                    <ChevronRight className="w-3 h-3" />
                    <span className="text-zinc-600">{item.next_destination}</span>
                  </div>
                </div>
              )}
            </motion.div>
